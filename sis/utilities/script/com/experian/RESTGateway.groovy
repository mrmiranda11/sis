 ...
 *
 */        
public class RESTGateway implements GroovyComponent<Message> {
    
    protected static final ExpLogger  logger       = new ExpLogger(this); 
    protected static final String     TR_SUCCESS   = "success";
    //protected static final Hashtable  REST_RESOURCES = {'da':'callDA','da/init':'getWSDL'}
    ...
    public String processMessage(final Message message, final Map<String, String> dataMap) throws Exception {
        ...
        int init = name.indexOf('/DAService/');
        String process = name.substring(init+11);
        ...
        if(message['initStrategy'] != null && message['initStrategy'] != ''){
            message.put("__ProcessFlowId","getWSDL")
            def strategy = message['initStrategy']?.trim()
